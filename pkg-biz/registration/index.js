...
const formatLastRegistrationTime = (value) => {
  if (!value) return '';
...
const safeParseCustomData = (value) => {
  if (!value) return {};
...
const normalizeSyncFieldValue = (value) => {
...
const buildLatestRegistrationSyncPayload = (registration, customFields) => {
  if (!registration || !Array.isArray(customFields) || customFields.length === 0) {
    return { values: {}, count: 0 };
  }

  const values = {};
  const customData = safeParseCustomData(registration.customData);
  ...
  customFields.forEach(field => {
    ...
    let nextValue = '';
    if (field.id === 'name') {
      nextValue = normalizeSyncFieldValue(registration.name);
    } else if (field.id === 'mobile') {
      nextValue = normalizeSyncFieldValue(registration.mobile);
    } else {
      nextValue = normalizeSyncFieldValue(customData[field.id]);
    }
...
Page({
  ...
  async loadLatestRegistrationInfo() {
    const { detail, customFields, isRegistered, showGroupSelection } = this.data;

    if (isRegistered || showGroupSelection || !Array.isArray(customFields) || customFields.length === 0) {
      this.setData({
        lastRegistration: null,
        lastRegistrationSyncValues: {},
        lastRegistrationSyncCount: 0,
    ...
    const seriesId = (detail && (detail.seriesId || detail.id)) ? String(detail.seriesId || detail.id).trim() : '';
    if (!seriesId) {
      this.setData({
        lastRegistration: null,
        lastRegistrationSyncValues: {},
    ...
    this.setData({ loadingLastRegistration: true });

    try {
      const result = await registrationAPI.getLatestBySeries(seriesId);
      const latestRegistration = result && result.code === 0 ? result.data : null;
      const syncPayload = buildLatestRegistrationSyncPayload(latestRegistration, customFields);

      this.setData({
        lastRegistration: latestRegistration,
        lastRegistrationSyncValues: syncPayload.values,
        lastRegistrationSyncCount: syncPayload.count,
        lastRegistrationTimeText: latestRegistration ? formatLastRegistrationTime(latestRegistration.registeredAt) : '',
        showLastRegistrationSync: !!latestRegistration && syncPayload.count > 0,
        loadingLastRegistration: false
      });
    } catch (error) {
      console.warn('加载上次报名信息失败:', error);
      this.setData({
        lastRegistration: null,
        lastRegistrationSyncValues: {},
        lastRegistrationSyncCount: 0,
  ...
  syncLastRegistration() {
    const { showLastRegistrationSync, lastRegistrationSyncValues, lastRegistrationSyncCount, formData } = this.data;
    if (!showLastRegistrationSync || !lastRegistrationSyncCount) {
      wx.showToast({ title: '暂无可同步信息', icon: 'none' });
      return;
    }
    ...
    wx.showToast({
      title: `已同步 ${lastRegistrationSyncCount} 项信息`,
      icon: 'success'
    });
  },
  ...
  onShareAppMessage() {
    ...
    return {
      ...
      path: `/pkg-biz/registration/index?id=${id}`,
