...
@Tag(name = "系统管理员接口", description = "系统管理员专属功能")
public class AdminController {
    ...
    @GetMapping("/me")
    @Operation(summary = "查询当前用户是否系统管理员", description = "仅用于前端决定是否展示系统管理入口，最终权限以后端白名单为准")
    ...
    @Operation(summary = "系统管理员查询活动列表", description = "系统管理员可查看系统内所有活动（不含草稿/待发布）")
    ...
    public ApiResponse<CreateActivityConfigVO> updateCreateActivityAdminOnly(
        ...
        String value = adminOnly ? "1" : "0";

        SystemSetting setting = systemSettingRepository.findById(KEY_CREATE_ACTIVITY_ADMIN_ONLY)
                .orElse(SystemSetting.builder()
                        .settingKey(KEY_CREATE_ACTIVITY_ADMIN_ONLY)
                        .settingValue(value)
